...
import 'src/gif_controller.dart';
import 'src/git_frame.dart';
import 'package:http/http.dart' as http;

export 'src/gif_controller.dart';
...
class GifViewState extends State<GifView> with TickerProviderStateMixin {
  ...
  void didUpdateWidget(covariant GifView oldWidget) {
    ...
    if (oldWidget.image != widget.image || controller.status == GifStatus.error) {
  ...
  Future<Uint8List?> _loadImageBytes(ImageProvider<Object> provider) {
    if (provider is NetworkImage) {
      ...
      return http.get(resolved, headers: provider.headers).then((value) => value.bodyBytes);
